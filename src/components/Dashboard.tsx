...
export const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  return (
    <div className="space-y-6">
      {/* Compact World Event Notification Pill if active */}
      {activeWorldEvent && (
        <div 
          onClick={() => setActiveTab('events')}
          className="cursor-pointer bg-red-950/30 border border-red-500/60 p-2.5 flex items-center justify-between text-xs hover:bg-red-900/30 transition-colors"
        >
          <div className="flex items-center gap-2 text-red-400">
            <Radio className="w-4 h-4 animate-pulse" />
            <span className="font-bold tracking-wider uppercase">
              ACTIVE CRISIS IN PROGRESS: {activeWorldEvent.title}
            </span>
          </div>
          <span className="text-emerald-400 font-bold uppercase tracking-widest text-[11px] hover:underline">
            [ VIEW CRISIS DOSSIER {'>>'} ]
          </span>
        </div>
      )}

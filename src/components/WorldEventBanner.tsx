...
import { Globe, AlertTriangle, TrendingUp, TrendingDown, Cpu, ShieldAlert, Radio, Clock, Zap } from 'lucide-react';
...
export const WorldEventBanner: React.FC<WorldEventBannerProps> = ({ event, compact = false }) => {
  ...
  if (compact) {
    return (
      <div className={`border ${theme.border} ${theme.bg} p-3 font-mono text-xs flex flex-wrap items-center justify-between gap-3 shadow-[0_0_12px_rgba(0,0,0,0.5)]`}>
        <div className="flex items-center gap-2 min-w-[220px]">
          <span className="relative flex h-2.5 w-2.5">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-red-500"></span>
          </span>
          <span className={`font-bold tracking-wider ${theme.text}`}>
  ...
  return (
    <div className={`relative bg-black border-2 ${theme.border} p-5 font-mono shadow-[0_0_20px_rgba(0,0,0,0.8)] overflow-hidden`}>
      {/* Top Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-emerald-500/20 pb-3 mb-3">
        <div className="flex items-center gap-3">
          <div className={`p-2 border ${theme.border} ${theme.bg}`}>
            {theme.icon}
          </div>
          <div>
            <div className="flex items-center gap-2">
            ...
            <h3 className={`text-lg font-bold tracking-widest mt-1 ${theme.text} glow-text`}>
              {event.title}
            </h3>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 self-end sm:self-center">
          <div className="text-right">
            <span className="text-[10px] text-zinc-500 block uppercase tracking-widest">CRISIS WINDOW</span>

...
export const WorldEventsView: React.FC<WorldEventsViewProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-emerald-500/30 pb-4">
        <div>
          ...
          <p className="text-xs text-emerald-500/70 mt-1">
            Real-time geopolitical intercepts, macroeconomic shocks, and covert telemetry broadcasts.
          </p>
        </div>
      ...
      {actionFeedback && (
        <div className="p-3 bg-emerald-950/40 border border-emerald-500 text-emerald-300 text-xs font-mono animate-pulse">
          {'>'} {actionFeedback}
        </div>
      )}
      ...
      {activeEvent ? (
        (() => {
          ...
          return (
            <div className={`relative bg-black border-2 ${theme.border} p-6 shadow-[0_0_25px_rgba(0,0,0,0.9)] space-y-5`}>
              {/* Event Header */}
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-emerald-500/20 pb-4">
                <div className="flex items-start gap-3.5">
                  <div className={`p-3 border ${theme.border} ${theme.bg}`}>
                    {theme.icon}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[11px] font-bold tracking-widest uppercase px-2 py-0.5 border ${theme.badge}`}>
                        LIVE CRISIS // {activeEvent.category.toUpperCase()} SECTOR
                      </span>
                    </div>
                    <h3 className={`text-2xl font-bold tracking-widest mt-1.5 ${theme.text} glow-text`}>
                      {activeEvent.title}
                    </h3>
                  </div>
                </div>

                <div className="bg-black border border-emerald-500/30 p-3 flex items-center gap-4 self-start md:self-center">
                  <div>
                    <span className="text-[10px] text-zinc-500 block uppercase tracking-widest">TRANSMISSION WINDOW</span>
              ...
              {/* Headline and Sitrep */}
              <div className="space-y-2 bg-zinc-950/80 p-4 border border-emerald-500/20">
                <div className="text-xs text-zinc-400 uppercase tracking-widest flex items-center gap-1.5">
                  <FileText className="w-3.5 h-3.5 text-emerald-400" /> SITUATION REPORT:
                </div>
                <p className="text-sm font-bold text-zinc-100 tracking-wide">
                  "{activeEvent.headline}"
                </p>
                <p className="text-xs text-emerald-400/80 leading-relaxed pt-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-black border border-emerald-500/30 p-3.5">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className={`text-lg font-bold ${activeEvent.creditMultiplier >= 1 ? 'text-emerald-400' : 'text-red-400'}`}>
                ...
                <div className="bg-black border border-emerald-500/30 p-3.5">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className={`text-lg font-bold ${activeEvent.intelMultiplier >= 1 ? 'text-cyan-400' : 'text-red-400'}`}>
                ...
                <div className="bg-black border border-emerald-500/30 p-3.5">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <Zap className={`w-5 h-5 ${activeEvent.tensionChange >= 0 ? 'text-amber-400' : 'text-blue-400'}`} />
                    <span className={`text-lg font-bold ${activeEvent.tensionChange >= 0 ? 'text-amber-400' : 'text-blue-400'}`}>
                      {activeEvent.tensionChange >= 0 ? `+${activeEvent.tensionChange}% Escalation` : `${activeEvent.tensionChange}% De-escalation`}
                    </span>
                  </div>
                  <div className="text-[10px] text-zinc-500 mt-1">
                    Direct impact on international alert state
                  </div>
                </div>
              </div>
                ...
                <div className="w-full bg-zinc-950 h-1.5 overflow-hidden border border-emerald-500/30">
                  <div
                    className={`h-full transition-all duration-1000 ${
                      progressPercent > 25 ? 'bg-emerald-500' : 'bg-red-500 animate-pulse'
                    }`}
              ...
              <div className="border-t border-emerald-500/20 pt-4 flex flex-col sm:flex-row items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto">
                  ...
                  >
                    [ LAUNCH DISINFORMATION (-50 INTEL) ]
                  </button>
                </div>
              </div>
            </div>
